...
class OFHandler (EventMixin):
  ...
  def _handle_PacketIn (self, event):
    ...
    #log.debug("switch id: %s" % event.dpid)

    if( event.port == 1 and pkt.type == ethernet.ARP_TYPE and pkt.next.opcode == arp.REQUEST):
        arp_req = pkt.next
        ...
        arp_reply.hwsrc = EthAddr(self.sw_info["eth1"][1])
...
def get_ip_setting():
  ...
  RTABLE.append( ('0.0.0.0', '10.0.1.1', '0.0.0.0', 'eth1') )
